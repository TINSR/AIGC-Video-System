...
export async function uploadToObjectStorage(
  ...
  try {
    ...
    const timeoutMs = Math.max(Number(process.env.OBJECT_STORAGE_UPLOAD_TIMEOUT_MS) || 30_000, 1_000);
    ...
    if (!response.ok) {
      return { ok: false, reason: `upload http ${response.status}` };
    }
    ...
    if (error instanceof Error && error.name === 'AbortError') {
      return { ok: false, reason: 'object storage upload timed out' };
    }
